...
describe('SortableDirective', () => {
	describe('Basic creation and lifecycle', () => {
		...
		it('should emit init event with Sortable instance', fakeAsync(() => {
		...
		it('should destroy Sortable instance on directive destroy', fakeAsync(() => {
			fixture.detectChanges();
			tick();
			const directive = getSortableDirective(fixture);
			const destroySpy = spyOn((directive as any).sortableInstance || {}, 'destroy').and.callThrough();
		...
		it('should clean up native event listeners on destroy', fakeAsync(() => {
			fixture.detectChanges();
			tick();
	...
	describe('Container selector', () => {
		it('should use inner container when container selector is provided', fakeAsync(() => {
		...
		it('should log error when container selector does not match', fakeAsync(() => {
			spyOn(console, 'error');
			TestBed.configureTestingModule({
			...
			expect(console.error).toHaveBeenCalledWith(
				jasmine.stringContaining('Container not found')
			);
		}));
	});

	describe('Individual option inputs', () => {
		it('should apply individual options to sortable instance', fakeAsync(() => {
			TestBed.configureTestingModule({
	...
	describe('Global config (GLOBALS injection token)', () => {
		it('should merge global config with local options', fakeAsync(() => {
			TestBed.configureTestingModule({
				imports: [
	...
	describe('Data binding types', () => {
		it('should work with FormArray', fakeAsync(() => {
			TestBed.configureTestingModule({
		...
		it('should work with signal arrays', fakeAsync(() => {
			TestBed.configureTestingModule({
	...
	describe('Manual mode (autoUpdateArray: false)', () => {
		...
		it('should not modify the array on update events', fakeAsync(() => {
			tick();
			const directive = getSortableDirective(fixture) as any;
			const instance = directive.sortableInstance;
			if (!instance) return;
		...
		it('should emit update event in manual mode', fakeAsync(() => {
			tick();
			const directive = getSortableDirective(fixture) as any;
			const instance = directive.sortableInstance;
			if (!instance) return;
		...
		it('should guard against duplicate onUpdate calls', fakeAsync(() => {
			tick();
			const directive = getSortableDirective(fixture) as any;
			const instance = directive.sortableInstance;
			if (!instance) return;
		...
		it('should reset dropEventProcessed on onStart', fakeAsync(() => {
			tick();
			const directive = getSortableDirective(fixture) as any;
			const instance = directive.sortableInstance;
			if (!instance) return;
		...
		it('should reset dropEventProcessed on onEnd', fakeAsync(() => {
			tick();
			const directive = getSortableDirective(fixture) as any;
			const instance = directive.sortableInstance;
			if (!instance) return;
		...
		it('should emit start event', fakeAsync(() => {
			tick();
			const directive = getSortableDirective(fixture) as any;
			const instance = directive.sortableInstance;
			if (!instance) return;
		...
		it('should emit end event', fakeAsync(() => {
			tick();
			const directive = getSortableDirective(fixture) as any;
			const instance = directive.sortableInstance;
			if (!instance) return;
		...
		it('should emit sort event', fakeAsync(() => {
			tick();
			const directive = getSortableDirective(fixture) as any;
			const instance = directive.sortableInstance;
			if (!instance) return;
		...
		it('should emit choose event', fakeAsync(() => {
			tick();
			const directive = getSortableDirective(fixture) as any;
			const instance = directive.sortableInstance;
			if (!instance) return;
		...
		it('should emit unchoose event', fakeAsync(() => {
			tick();
			const directive = getSortableDirective(fixture) as any;
			const instance = directive.sortableInstance;
			if (!instance) return;
		...
		it('should emit clone event', fakeAsync(() => {
			tick();
			const directive = getSortableDirective(fixture) as any;
			const instance = directive.sortableInstance;
			if (!instance) return;
	...
	describe('Auto mode (autoUpdateArray: true)', () => {
		...
		it('should reorder array on onUpdate', fakeAsync(() => {
			tick();
			const directive = getSortableDirective(fixture) as any;
			const instance = directive.sortableInstance;
			if (!instance) return;
		...
		it('should set up transfer callback on onAdd', fakeAsync(() => {
			tick();
		...
		it('should handle onAdd with undefined newIndex', fakeAsync(() => {
			tick();
			const directive = getSortableDirective(fixture) as any;
			const instance = directive.sortableInstance;
			if (!instance) return;
		...
		it('should handle onRemove with undefined oldIndex', fakeAsync(() => {
			tick();
			const directive = getSortableDirective(fixture) as any;
			const instance = directive.sortableInstance;
			if (!instance) return;
		...
		it('should handle onUpdate with undefined indexes', fakeAsync(() => {
			tick();
			const directive = getSortableDirective(fixture) as any;
			const instance = directive.sortableInstance;
			if (!instance) return;
	...
	describe('Clone mode handling (manual mode)', () => {
		...
		it('should handle onAdd in manual mode with clone (remove item from target)', fakeAsync(() => {
			tick();
			const directive = getSortableDirective(fixture) as any;
			const instance = directive.sortableInstance;
			if (!instance) return;
		...
		it('should handle onAdd in manual mode without clone (revert to source)', fakeAsync(() => {
			tick();
			const directive = getSortableDirective(fixture) as any;
			const instance = directive.sortableInstance;
			if (!instance) return;
		...
		it('should restore original element in source on onRemove in manual clone mode', fakeAsync(() => {
			tick();
			const directive = getSortableDirective(fixture) as any;
			const instance = directive.sortableInstance;
			if (!instance) return;
		...
		it('should handle onRemove when item still has parent in manual clone mode', fakeAsync(() => {
			tick();
			const directive = getSortableDirective(fixture) as any;
			const instance = directive.sortableInstance;
			if (!instance) return;
		...
		it('should not crash on onRemove when clone has no parent', fakeAsync(() => {
			tick();
			const directive = getSortableDirective(fixture) as any;
			const instance = directive.sortableInstance;
			if (!instance) return;
		...
		it('should guard against duplicate onAdd calls in manual mode', fakeAsync(() => {
			tick();
			const directive = getSortableDirective(fixture) as any;
			const instance = directive.sortableInstance;
			if (!instance) return;
	...
	describe('Auto mode clone handling (onRemove)', () => {
		...
		it('should use cloneFunction when cloning items', fakeAsync(() => {
			tick();
			const directive = getSortableDirective(fixture) as any;
			const instance = directive.sortableInstance;
			if (!instance) return;
	...
	describe('User-provided event callbacks in options', () => {
		it('should call user onStart and onEnd callbacks', fakeAsync(() => {
			TestBed.configureTestingModule({
	...
	describe('ngOnChanges', () => {
		it('should update sortable options when options input changes', fakeAsync(() => {
			TestBed.configureTestingModule({
				imports: [SortableModule, BasicTestComponent]
			});
			const fixture = TestBed.createComponent(BasicTestComponent);
			...
			const optionSpy = spyOn(instance, 'option');
		...
		it('should not update on first change', fakeAsync(() => {
			TestBed.configureTestingModule({
				imports: [SortableModule, BasicTestComponent]
			});
			const fixture = TestBed.createComponent(BasicTestComponent);
	...
	describe('Native event suppression', () => {
		it('should suppress native sortable events on the container', fakeAsync(() => {
			TestBed.configureTestingModule({
				imports: [SortableModule, BasicTestComponent]
			});
			const fixture = TestBed.createComponent(BasicTestComponent);
	...
	describe('Move event', () => {
		it('should emit move event with event and originalEvent', fakeAsync(() => {
			TestBed.configureTestingModule({
				imports: [SortableModule, BasicTestComponent]
			});
			const fixture = TestBed.createComponent(BasicTestComponent);
	...
	describe('revertSortableDom', () => {
		it('should revert DOM when item moved down (newIndex > oldIndex)', fakeAsync(() => {
			TestBed.configureTestingModule({
				imports: [SortableModule, ManualModeTestComponent]
			});
		...
		it('should not revert when oldIndex equals newIndex', fakeAsync(() => {
			TestBed.configureTestingModule({
				imports: [SortableModule, ManualModeTestComponent]
			});
			const fixture = TestBed.createComponent(ManualModeTestComponent);
	...
	describe('revertTransferDom (non-clone manual mode)', () => {
		it('should move item back to source container', fakeAsync(() => {
			TestBed.configureTestingModule({
				imports: [SortableModule, ManualModeTestComponent]
			});
			const fixture = TestBed.createComponent(ManualModeTestComponent);
	...
	describe('Auto mode transfer flow (onAdd + onRemove coordination)', () => {
		...
		it('should invoke transfer callback from onRemove in non-clone auto mode', fakeAsync(() => {
			tick();
			const directive = getSortableDirective(fixture) as any;
			const instance = directive.sortableInstance;
			if (!instance) return;
		...
		it('should execute the full onAdd auto-mode transfer callback', fakeAsync(() => {
			tick();
			const directive = getSortableDirective(fixture) as any;
			const instance = directive.sortableInstance;
			if (!instance) return;
	...
	describe('isCloning getter paths', () => {
		it('should detect clone via checkPull function', fakeAsync(() => {
			@Component({
				template: `
					<div [hubSortable]="items"
						 [options]="options"
		...
		it('should return false when group is not set', fakeAsync(() => {
			@Component({
				template: `
	...
	describe('applyIndividualOptionChanges', () => {
		it('should apply individual input changes to the sortable instance', fakeAsync(() => {
			TestBed.configureTestingModule({
				imports: [SortableModule, IndividualOptionsTestComponent]
			});
			const fixture = TestBed.createComponent(IndividualOptionsTestComponent);
	...
	describe('revertTransferDom edge case', () => {
		it('should not revert when oldIndex is undefined', fakeAsync(() => {
			TestBed.configureTestingModule({
				imports: [SortableModule, ManualModeTestComponent]
			});
			const fixture = TestBed.createComponent(ManualModeTestComponent);
	...
	describe('getBindings with different input types', () => {
		it('should pass through a SortableBindings instance directly', fakeAsync(() => {
			TestBed.configureTestingModule({
				imports: [SortableModule, BasicTestComponent]
			});
			const fixture = TestBed.createComponent(BasicTestComponent);
		...
		it('should return empty bindings when items is undefined', fakeAsync(() => {
			@Component({
		...
		it('should return SortableBindings instance if passed directly', fakeAsync(() => {
			TestBed.configureTestingModule({
				imports: [SortableModule, BasicTestComponent]
			});
			const fixture = TestBed.createComponent(BasicTestComponent);
	...
	describe('isCloning fallback branches', () => {
		it('should return false when group has no checkPull and pull is not clone', fakeAsync(() => {
			@Component({
				template: `
					<div [hubSortable]="items"
		...
		it('should return false when checkPull throws an error', fakeAsync(() => {
			TestBed.configureTestingModule({
				imports: [SortableModule, BasicTestComponent]
			});
			const fixture = TestBed.createComponent(BasicTestComponent);
		...
		it('should return false when group is null/undefined', fakeAsync(() => {
			TestBed.configureTestingModule({
				imports: [SortableModule, BasicTestComponent]
			});
			const fixture = TestBed.createComponent(BasicTestComponent);
		...
		it('should detect clone when group.pull is directly set to clone string', fakeAsync(() => {
			TestBed.configureTestingModule({
				imports: [SortableModule, BasicTestComponent]
			});
			const fixture = TestBed.createComponent(BasicTestComponent);
	...
	describe('revertSortableDom boundary conditions', () => {
		it('should handle item moved up when refChild is at end (|| null fallback)', fakeAsync(() => {
			TestBed.configureTestingModule({
				imports: [SortableModule, ManualModeTestComponent]
			});
			const fixture = TestBed.createComponent(ManualModeTestComponent);
	...
	describe('revertTransferDom boundary conditions', () => {
		it('should handle revert when source has no child at oldIndex (|| null fallback)', fakeAsync(() => {
			TestBed.configureTestingModule({
				imports: [SortableModule, ManualModeTestComponent]
			});
			const fixture = TestBed.createComponent(ManualModeTestComponent);
	...
	describe('proxyEvent with no user handler', () => {
		it('should work when no user handler is defined for an event', fakeAsync(() => {
			@Component({
				template: `
	...
	describe('optionsWithoutEvents merging', () => {
		it('should handle null globalConfig gracefully', fakeAsync(() => {
		...
		it('should handle undefined options input', fakeAsync(() => {
			@Component({
				template: `
	...
	describe('Edge cases', () => {
		it('should handle null items input gracefully', fakeAsync(() => {
			@Component({
		...
		it('should handle undefined items input gracefully', fakeAsync(() => {
			@Component({
